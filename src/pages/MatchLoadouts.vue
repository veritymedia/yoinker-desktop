<template>
  <div>
    ...
    <div class="lastUpdateDiv">
      ...
      <span v-if="showTime" class="lastUpdateValue">{{
        lastUpdateString
      }}</span>
      <div v-else class="noMatch">
        <span class="lastUpdateValue red"
          >Couldn't fetch match or no match found in cache! Refresh the website
          or download vRY below</span
        >
...
export default {
  ...
  methods: {
    ...
    closeModal() {
      (this.showModal = false), (this.modalArguments = null);
    },

    lastUpdatedLoop() {
      setInterval(() => {
        this.lastUpdateSeconds = Math.round(
          +new Date() / 1000 - this.lastUpdate
        );
    ...
    getVersion() {
      fetch(
        "https://api.github.com/repos/isaacKenyon/VALORANT-rank-yoinker/releases"
      )
